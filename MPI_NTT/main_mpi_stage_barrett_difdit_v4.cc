...
static inline u64 mul_mod_barrett(u64 x, u64 y, const BarrettReducer &br) {
...
void ntt_serial(std::vector<u64> &f, u64 mod, u64 root, bool inverse) {
    int n = (int)f.size();
    bit_reverse(f);
    BarrettReducer br = make_barrett(mod);
    ...
    for (int len = 2; len <= n; len <<= 1) {
        ...
        for (int k = 1; k < half; ++k) {
            roots[k] = mul_mod_barrett(roots[k - 1], wn, br);
        }
...
#pragma omp simd
        for (int i = 0; i < n; ++i) {
            f[i] = mul_mod_barrett(f[i], inv_n, br);
        }
    }
}
...
void multiply_mod_ntt(const u64 *a, const u64 *b, std::vector<u64> &out, int n, NttMod nm) {
    ...
    BarrettReducer br = make_barrett(nm.mod);
...
#pragma omp simd
    for (int i = 0; i < limit; ++i) {
        A[i] = mul_mod_barrett(A[i], B[i], br);
    }
...
void ntt_mpi_dif_forward(std::vector<u64> &f, u64 mod, u64 root) {
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
...
#pragma omp simd
            for (int k = 0; k < half; ++k) {
                ...
                u64 y = f[base + k + half];
                f[base + k] = add_mod(x, y, mod);
                f[base + k + half] = mul_mod_barrett(sub_mod(x, y, mod), roots[k], br);
            }
        }
...
void intt_mpi_dit_inverse(std::vector<u64> &f, u64 mod, u64 root) {
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        std::cout << "MPI stage + Barrett + DIF/DIT version, processes = " << size
