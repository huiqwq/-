...
void multiply_mod_ntt(const u64 *a, const u64 *b, std::vector<u64> &out,
                      int n, NttMod nm) {
    int limit = 1;
...
void prepare_crt_inverse(u64 inv_prod_mod[CRT_CNT]) {
    ...
    for (int i = 1; i < CRT_CNT; ++i) {
        inv_prod_mod[i] =
            qpow((u64)(prod % CRT_MODS[i].mod), CRT_MODS[i].mod - 2,
                 CRT_MODS[i].mod);
        prod *= CRT_MODS[i].mod;
    }
}

u64 crt_combine_one(const u64 r[CRT_CNT], u64 target_mod,
...
void poly_multiply_crt_serial(u64 *a, u64 *b, u64 *ab, int n, u64 target_mod) {
    ...
    for (int mod_id = 0; mod_id < CRT_CNT; ++mod_id) {
        multiply_mod_ntt(a, b, residues[mod_id], n, CRT_MODS[mod_id]);
    }
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "serial CRT NTT baseline, repeat = " << repeat << std::endl;
    ...
    for (int i = test_begin; i <= test_end; ++i) {
        ...
        for (int rep = 0; rep < repeat; ++rep) {
            ...
            poly_multiply_crt_serial(a, b, ab, n_, p_);
