...
void ntt_dif_forward_openmp_simd(std::vector<u64> &f, u64 mod, u64 root) {
    int n = (int)f.size();
...
#pragma omp simd
            for (int k = 0; k < half; ++k) {
                ...
                u64 y = f[base + k + half];
                f[base + k] = add_mod(x, y, mod);
                f[base + k + half] = mul_mod_ntt(sub_mod(x, y, mod), roots[k], mod);
            }
        }
    }
}

void intt_dit_inverse_openmp_simd(std::vector<u64> &f, u64 mod, u64 root) {
    int n = (int)f.size();
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        std::cout << "MPI CRT mod-parallel + OpenMP/SIMD + DIF/DIT version, processes = " << size
